...
export const createVisitSchema = z
  .object({
    ...
    employee_id: z.preprocess(
      (v) => (v === "" ? null : v),
      z.string().uuid("Employee is required").optional().nullable()
    ),
    branch_id: z.string().uuid("Branch is required"),
    care_plan_id: z.preprocess(
      (v) => (v === "" ? null : v),
    ...
    scheduled_date: z
      .string()
      .refine((date) => !isNaN(Date.parse(date)), "Visit date is required and must be valid"),
    start_time: z
      .string()
      .refine(
...
export const updateVisitSchema = z
  .object({
    ...
    scheduled_date: z
      .string()
      .refine((date) => !isNaN(Date.parse(date)), "Visit date must be valid")
      .optional(),
    start_time: z
      .string()
      .refine(
