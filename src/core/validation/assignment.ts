...
export const createAssignmentSchema = z
  .object({
    ...
    assigned_from: z
      .string()
      .refine((date) => !isNaN(Date.parse(date)), "Start date is required and must be valid"),
    assigned_until: z
      .string()
      .optional()
      .nullable()
      .refine((date) => !date || !isNaN(Date.parse(date)), "End date must be a valid date"),
...
export const updateAssignmentSchema = z
  .object({
    ...
    notes: z.string().max(5000, "Notes cannot exceed 5000 characters").optional().nullable(),
    // Lifecycle flag, not assignment content - lets Activate reverse an
    // Archive (DELETE) without a separate endpoint. Previously nothing could
    // clear is_deleted once set, so archiving was permanently irreversible.
